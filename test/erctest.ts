  ...
  let account3 :any;
    ...
    describe("Whitelist",function(){
        ...
        it("Fourth: NOT to be able to run function if you're not whitelisted", async function(){
            const [asdf] = await ethers.getSigners();
            const addr = await erc.exampleFunction();
            expect(addr).to.be.equal(false);
        })
    ...
    describe("Mint", function(){
        ...
        it("NOT to be able to mint tokens if you're NOT whitelisted", async function(){
            const mintable = erc.mint(erc.connect(buyer).address, 1000);
    ...
    describe("Burn", async function(){
        ...
        it("NOT be able to burn tokens if you're not whitelisted", async function(){
            const burnable = erc.burn(erc.connect(buyer).address, 500);
    ...
    describe("Transactions", function(){
        ...
        it("Should be possible to transfer tokens from owner to other account", async function(){
            const addOwner = await erc.addUser(erc.address);
            const OwnerMint = await erc.mint(erc.address, 1000);  
        })
    })
})